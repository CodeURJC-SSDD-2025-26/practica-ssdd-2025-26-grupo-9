...
public class UserLoginService {
    ...
    public ResponseEntity<AuthResponse> refresh(HttpServletResponse response, String refreshToken) {
        try {
            ...
            UserDetailsService userDetailsServiceLocal = this.userDetailsService;
            UserDetails user = userDetailsServiceLocal.loadUserByUsername(claims.getSubject());
            ...
            AuthResponse loginResponse = new AuthResponse(AuthResponse.Status.FAILURE, "Failure while processing refresh token");
            return ResponseEntity.ok().body(loginResponse);
        }
    }
